<?php 
	function getInfo() {
		...
		<h1>Черешня</h1>
		<p>...А Вы знали, что <strong>черешня</strong> является естественным источником мелатонина, гормона, который помогает регулировать цикл сна - бодрствования человека</p>
		</div>';
	}
...
<form action="<?php echo $_SERVER["PHP_SELF"];?>" method="post">
	<label>Ваше имя:<input type="text" name="name" maxlength="15" pattern="[A-zА-я]{1,15}[-]?[A-zА-я]{0,15}" title="Введите тлько буквы!"></label>
	<hr>
	<p>Черешня растет...</p>
	<input type="radio" value="1" name="s1"> на деревьях
	<input type="radio" value="2" name="s1"> в земле
	<hr>
	<p>Период созревания черешни?</p>
	<input type="radio" value="1" name="s2"> май - июнь
...
function processingData() {
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		if (empty($_POST['name'])||empty($_POST['s1'])||empty($_POST['s2'])) {
			...
			if ($points>10){$text_mess .=", вы обожаете черешню! <br>";}
		}
	}
	return $text_mess;
